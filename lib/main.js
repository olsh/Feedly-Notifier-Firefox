...
/* Opens feedly site and and add listener on all http requests */
function getAccessToken() {
    events.on("http-on-modify-request", getAccessTokenFromRequest);
    openFeedlyTab();
}

/* Trying get access token from http requests,
 * if success then removes listener on http requests,
 * stores access token in the storage and initializes scheduler */
function getAccessTokenFromRequest(event) {
    ...
    try {
        var header = channel.getRequestHeader("Cookie");
