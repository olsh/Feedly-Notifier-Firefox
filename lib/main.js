...
exports.onUnload = function (reason) {
    ...
    if (reason !== "shutdown") {
        while (!handlerFound) {
            handlerNumber++;
            handlerTitle = prefs.get(appGlobal.subscribeHandlerConstants.keyPrefix + handlerNumber + appGlobal.subscribeHandlerConstants.titleKey);
            if (!handlerTitle) {
                break;
            }
        ...
        if (handlerFound) {
            ...
            prefs.reset(appGlobal.subscribeHandlerConstants.keyPrefix + handlerNumber + appGlobal.subscribeHandlerConstants.titleKey);
            prefs.reset(appGlobal.subscribeHandlerConstants.keyPrefix + handlerNumber + appGlobal.subscribeHandlerConstants.typeVal);
            prefs.reset(appGlobal.subscribeHandlerConstants.keyPrefix + handlerNumber + appGlobal.subscribeHandlerConstants.uriKey);

            // If it's set as the "always use", clear that.
            // TODO: Figure out how to make this work without a restart
            if (prefs.get(appGlobal.subscribeHandlerConstants.autoKey) === appGlobal.subscribeHandlerConstants.uriVal) {
                prefs.reset(appGlobal.subscribeHandlerConstants.autoKey);
            }
