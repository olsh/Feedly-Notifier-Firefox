...
exports.ToolbarButton = Class({
  initialize: function(options) {
    ...
    winUtils.WindowTracker({
      onTrack: function (window) {
        ...
        tbb.setAttribute('badge', safeOptions.badge);
        tbb.addEventListener("command", TBB_NS(self).commandFunction, true);
  ...
  set tooltiptext(value) {
    ...
    setBadge: function setBadge(value) {
        let val = TBB_NS(this).options.badge = value > 0 ? value : "";
        getToolbarButtons(function (tbb) {
            tbb.setAttribute('badge', val);
        }, this.id);
        return val;
    },
    get badge() TBB_NS(this).options.badge,
    set badge(value) this.setBadge(value),
  setListeners: function setListeners(settings){
      var oldCommandFunction = TBB_NS(this).commandFunction;
      let newCommandFunction = TBB_NS(this).commandFunction = function() {
          if (settings.onCommand)
          {
              settings.onCommand({}); // TODO: provide something?
          }

          if (settings.panel) {
              settings.panel.show(this);
          }
      }
      getToolbarButtons(function (tbb) {
          tbb.removeEventListener("command", oldCommandFunction, true);
          tbb.addEventListener("command", newCommandFunction, true);
      }, this.id);
  }
});
